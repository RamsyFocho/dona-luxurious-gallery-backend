...
export const bulkCreateCategories = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (existingCategories.length > 0) {
      return next(
        new AppError("Some categories with these slugs already exist", 400)
      );
    }
...
export const updateCategory = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (updateData.slug && updateData.slug !== req.params.slug) {
      ...
      if (existingCategory) {
        return next(
          new AppError("Category with this slug already exists", 400)
        );
      }
    }
